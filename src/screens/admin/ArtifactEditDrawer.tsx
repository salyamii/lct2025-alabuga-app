...
import { ArtifactUpdateRequest, ArtifactRarityEnum } from "../../api/types/apiTypes";
import { Artifact } from "../../domain/artifact";
import { mediaService } from "../../api/services/mediaService";

interface ArtifactEditDrawerProps {
  artifact: Artifact | null;
}

export function ArtifactEditDrawer({ artifact }: ArtifactEditDrawerProps) {
  const { artifactEditOpen, closeArtifactEdit } = useOverlayStore();
  ...
  useEffect(() => {
    if (artifact) {
      ...
      setUploadedImageUrl(artifact.imageUrl || "");
    }
  }, [artifact]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      setIsUpdating(true);
      const artifactData: ArtifactUpdateRequest = {
      ...
      toast.error("Ошибка при обновлении артефакта. Попробуйте еще раз.");
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  return (
    <Drawer open={artifactEditOpen} onOpenChange={handleClose}>
      <DrawerContent className="max-w-2xl mx-auto max-h-[90vh] flex flex-col">
        <DrawerHeader className="flex-shrink-0">
          ...
          <DrawerDescription>
            Измените информацию об артефакте
          </DrawerDescription>
        </DrawerHeader>
        ...
        <div className="flex-shrink-0 px-6 pb-6">
          <div className="flex justify-end gap-3">
            ...
            <Button 
              ...
              disabled={isUpdating || !formData.title.trim() || !formData.description.trim() || !formData.rarity}
              className="bg-primary hover:bg-primary-600"
            >
              {isUpdating ? "Обновление..." : "Обновить артефакт"}
            </Button>
          </div>
        </div>
      </DrawerContent>
