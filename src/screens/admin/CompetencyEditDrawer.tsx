...
export function CompetencyEditDrawer({
  open,
  onOpenChange,
  competency,
}: CompetencyEditDrawerProps) {
  ...
  // Заполняем форму данными компетенции при открытии
  useEffect(() => {
    if (competency && open) {
      setFormData({
                  ...
                  <CardContent className="space-y-3 text-sm">
                    ...
                    <div className="pt-2 border-t border-border">
                      <div className="text-xs text-muted-foreground">
                        <strong>ID компетенции:</strong> {competency.id}
                      </div>
                    </div>
                  </CardContent>
                </Card>
