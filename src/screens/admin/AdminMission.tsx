...
export function AdminMission({
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        await Promise.all([handleFetchMissions(), handleFetchMissionChains(), fetchRanks()]);
      } catch (error) {
  ...
  // Функция для анализа рангов в цепочке миссий
  const analyzeChainRanks = (chain: MissionChain) => {
    // Если в цепочке нет миссий - некорректная
    if (chain.missions.length === 0) {
      return {
        isValid: false,
        reason: "Нет миссий",
        ranks: [],
        rankText: "Некорректная цепочка",
        color: "bg-red-500"
      };
    }

    // Получаем уникальные ранги из миссий
    const requiredRanks = chain.missions.map(mission => mission.rankRequirement);
    const uniqueRanks = Array.from(new Set(requiredRanks));
    
    // Если несколько разных рангов - некорректная
    ...
    // Если один ранг - корректная
    const rankId = uniqueRanks[0];
    ...
    return {
      ...
      reason: null,
      ranks: [rankId],
      rankText: rank ? rank.name : `Ранг #${rankId}`,
      color: "bg-green-500"
    };
  };
  ...
  const getMissionDependencies = (missionId: number, chain: MissionChain) => {
    return chain.dependencies
      ...
      .map((dep) => {
        const prerequisiteMission = missions.find(
          (m) => m.id === dep.prerequisiteMissionId
        );
        return (
          prerequisiteMission?.title || `Миссия #${dep.prerequisiteMissionId}`
        );
      });
  };
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-6 text-sm text-muted-foreground">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              {mission.rewardArtifacts.length +
                                mission.rewardCompetencies.length +
                                mission.rewardSkills.length}{" "}
                              наград
                            </span>
                          </div>
                          <div className="mt-2">
              ...
              {isLoading ? (
                ...
                <div className="space-y-4">
                  {missionChains.map((chain) => {
                    const rankAnalysis = analyzeChainRanks(chain);
                      ...
                      <div key={chain.id} className="admin-card p-4 rounded-lg">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-start gap-4">
                            <div className="w-12 h-12 bg-gradient-to-br from-info to-soft-cyan rounded-lg flex items-center justify-center flex-shrink-0">
                            ...
                            <div className="space-y-2">
                              <div>
                                <div className="flex items-center gap-3 mb-1">
                                  ...
                                  <div
                                    className={`w-2 h-2 rounded-full ${rankAnalysis.color}`}
                                    title={rankAnalysis.isValid ? rankAnalysis.rankText : `Некорректная: ${rankAnalysis.reason}`}
                                  />
                                </div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                <div>
                                  ...
                                  <span className={`ml-1 font-medium ${rankAnalysis.isValid ? 'text-green-600' : 'text-red-600'}`}>
                                    {rankAnalysis.isValid ? rankAnalysis.rankText : 'Некорректная'}
                                  </span>
                                </div>
                                <div>
                                  <span className="text-muted-foreground">
                            ...
                            <TooltipProvider>
                              <div className="flex items-center gap-2 overflow-x-auto pb-2">
                                {orderedMissions.map((mission, index) => {
                                  const dependencies = getMissionDependencies(
                                    mission.id,
                                    chain
                                  );
                                  return (
                                      ...
                                      <Tooltip>
                                        ...
                                        <TooltipContent className="bg-slate-900/95 border border-slate-600/60 text-slate-100 shadow-2xl backdrop-blur-sm">
                                          <div className="max-w-xs">
                                            <div className="font-semibold mb-2 text-slate-50 text-sm">
                                              {mission.title}
                                            </div>
                                            {dependencies.length > 0 ? (
                                              <div>
